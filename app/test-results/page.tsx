...
import { fetchPatientAssessments, isCompletedAssessment, type PatientAssessment } from "@/lib/patient-assessments"
...
export default function TestResultsPage() {
  ...
  const [rows, setRows] = useState<PatientAssessment[]>([])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const list = await fetchPatientAssessments(patientId)
        setRows(list.filter(isCompletedAssessment))
      } catch (err) {
  ...
  const avgCompletionRate = useMemo(() => {
    ...
    const total = rows.reduce((sum, item) => {
      ...
      return sum + (item.completed_questions / item.total_questions) * 100
    }, 0)
    return Number((total / rows.length).toFixed(0))
  }, [rows])
          ...
          <div className="space-y-4">
            {rows.map((item) => (
              <ResultListItem
                ...
                testName={item.test}
                completionDate={null}
                score={item.total_questions ? (item.completed_questions / item.total_questions) * 100 : null}
                duration={null}
