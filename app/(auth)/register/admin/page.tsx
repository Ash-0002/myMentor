...
import { registerHospitalAdminJson } from "@/lib/auth-service"
import { API_TENANT_NAME } from "@/lib/backend-api"
...
export default function AdminRegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const required = [
      ...
      formData.hospital_phone,
      formData.hospital_email,
    ...
    try {
      const response = await registerHospitalAdminJson({
        tenant_name: API_TENANT_NAME,
        ...
        hospital_address: formData.hospital_address.trim(),
        hospital_phone: formData.hospital_phone.trim(),
        hospital_email: formData.hospital_email.trim(),
  ...
  const inputClass =
    "w-full h-11 px-4 rounded-xl border border-input bg-background/50 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none text-sm font-medium"
        ...
        <div className="w-full max-w-2xl">
          <Card className="p-10 border-border/50 shadow-2xl shadow-primary/5 bg-card/80 backdrop-blur-xl">
            <div className="space-y-2 mb-10 text-center">
              <h1 className="text-2xl font-bold tracking-tight">Hospital Admin Registration</h1>
              <p className="text-muted-foreground text-sm">
                Create your admin account and register your hospital
              </p>
            </div>

            {error && (
              <div className="p-4 mb-8 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm font-medium">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleRegister} className="space-y-8">
              <div className="space-y-6">
                <div className="flex items-center gap-3 border-b border-border pb-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-primary/70">Admin Account</span>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 border-b border-border pb-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-primary/70">Hospital Details</span>
                </div>
                <div className="grid md:grid-cols-2 gap-x-6 gap-y-5">
                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground ml-1">Hospital Name *</label>
                    <input type="text" name="hospital_name" value={formData.hospital_name} onChange={handleChange} className={inputClass} required />
                  </div>
                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground ml-1">Hospital Address *</label>
                    <input type="text" name="hospital_address" value={formData.hospital_address} onChange={handleChange} className={inputClass} required />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground ml-1">Hospital Phone *</label>
                    <input type="tel" name="hospital_phone" value={formData.hospital_phone} onChange={handleChange} className={inputClass} required />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground ml-1">Hospital Email *</label>
                    <input type="email" name="hospital_email" value={formData.hospital_email} onChange={handleChange} className={inputClass} required />
                  </div>
                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground ml-1">Doctor Name *</label>
                    <input type="text" name="doctor_name" value={formData.doctor_name} onChange={handleChange} className={inputClass} required />
                  </div>
                </div>
              </div>
              ...
              <Button type="submit" className="w-full h-12 text-sm font-bold shadow-lg shadow-primary/20 rounded-xl" disabled={loading}>
                {loading ? "Creating hospital & admin..." : "Create Admin Account"}
              </Button>
            </form>
          </Card>
        </div>
